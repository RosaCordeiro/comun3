...
Boolean ivb_Selecao_Linhas_padrao = False

Boolean ivb_Ordena_Colunas = False
Boolean ExibeMensagem = True
String MensagemErro = ""
Boolean ivb_ddw_MultiSelecao = False
String ivs_ddw_MultiSelecao_Coluna = ''
Private String ivs_ddw_MultiSelecao_Ultima_Coluna = ''
...
public subroutine of_ddw_multiselecao ()
public function string of_ddw_multiselecao_get (string ps_separador)
public subroutine of_ddw_multiselecao_focus (string ps_getfocus)
...
public function boolean of_changedataobject (string ps_dataobject);//Por padr$$HEX1$$e300$$ENDHEX$$o mostra mensagem de erro em tela
Return This.Of_ChangeDataObject(ps_dataobject, True)
end function
...
public subroutine of_ddw_multiselecao ();/* ==================================================
Esta fun$$HEX2$$e700e300$$ENDHEX$$o controla a sele$$HEX2$$e700e300$$ENDHEX$$o de DropDownDataWindow com CheckBox para multisele$$HEX2$$e700e300$$ENDHEX$$o de itens.
-- Exemplos de funcionamento:
1. Ao selecionar o primeiro da lista, desmarca os outros.
2. Ao selecionar qualquer outro, desmarca o primeiro.
3. Ao selecionar algum j$$HEX1$$e100$$ENDHEX$$ marcado, desmarca (mant$$HEX1$$e900$$ENDHEX$$m sempre um marcado).

Como utilizar:
Chamar no ItemChanged da DataWindow quando alterar o valor de uma coluna ddw.
-- A primeira linha da ddw deve conter um item na lista para selecionar todos.
	SELECT 	'Todos' de_item, 'S' id_selecionado
-- Pode fazer Union com cada item, ou buscar os itens de uma tabela.
	UNION
	SELECT 	de_item, 'N' id_selecionado
	FROM tabela

Checkbox: id_selecionado 'S' ou 'N'
Marcar "Always Show List" no campo ddw da dw

of_ddw_multiselecao_focus()
of_ddw_multiselecao_get( /*ref string ps_itens[] */)
*/

Long lvl_For
DataWindowChild lvdwc

If This.GetChild(ivs_ddw_MultiSelecao_Coluna, lvdwc) = -1 Then Return
If Long(lvdwc.Describe('id_selecionado.id')) = 0 Then Return // Coluna existe?
		
Choose Case lvdwc.GetRow()
	Case 1 // 'Todos'
		lvdwc.SetItem(1,  'id_selecionado', 'S') // Marca o checkbox do primeiro item
		For lvl_For = 2 To lvdwc.RowCount()
			lvdwc.SetItem(lvl_For,  'id_selecionado', 'N') // Desmarca os outros
		Next
	Case is > 0 // Outros
		lvdwc.SetItem(1,  'id_selecionado', 'N') // Desmarca o primeiro
		// Marca ou desmarca
		If lvdwc.GetItemString(lvdwc.GetRow(), 'id_selecionado') = 'N' Then 
			lvdwc.SetItem(lvdwc.GetRow(), 'id_selecionado', 'S')
		Else
			lvdwc.SetItem(lvdwc.GetRow(),  'id_selecionado', 'N')
		End If
End Choose
end subroutine

public function string of_ddw_multiselecao_get (string ps_separador);/*
Esta fun$$HEX2$$e700e300$$ENDHEX$$o retorna os itens marcados na ddw na vari$$HEX1$$e100$$ENDHEX$$vel ref lvs_Itens[].

of_ddw_multiselecao()
of_ddw_multiselecao_focus()
*/

Boolean lvb_Todos = False
Long lvl_For
String lvs_Itens[], lvs_Itens_Sel
DataWindowChild lvdwc

// Percorrer ddw para pegar os marcados
If This.GetChild(ivs_ddw_MultiSelecao_Coluna, lvdwc) > 0 Then
	lvb_Todos = (lvdwc.GetItemString(1, 'id_selecionado') = 'S') // 'Todos' (pega o primeiro item)
	// Percorre os demais
	For lvl_For = 2 To lvdwc.RowCount()
		// Popula o array com os itens selecionados (ou com todos)
		If lvdwc.GetItemString(lvl_For, 'id_selecionado') = 'S' Or lvb_Todos Then
			lvs_Itens[UpperBound(lvs_Itens) +1] = lvdwc.GetItemString(lvl_For, ivs_ddw_MultiSelecao_Coluna)
		End If
 	Next
End If

If lvb_Todos Then
	lvs_Itens_Sel = lvdwc.GetItemString(1, ivs_ddw_MultiSelecao_Coluna) // Pega o primeiro item
Else
	lvs_Itens_Sel = gf_Concatenar_Array(lvs_Itens, ps_Separador)
End If

Return lvs_Itens_Sel // True quando retornou todos.
end function

public subroutine of_ddw_multiselecao_focus (string ps_getfocus);/*
Chamada no ItemFocusChanged quando dwo.Name = ivs_Coluna_ddw_MultiSelecao + '_sel' 

of_ddw_multiselecao()
of_ddw_multiselecao_get( /*ref string ps_itens[] */)
*/

String lvs_Coluna_Sel

lvs_Coluna_Sel = ivs_ddw_MultiSelecao_Coluna + "_sel"

// Quando selecionar o campo de itens selecionados, esconder ele e mostrar a ddw depois
If ps_GetFocus = lvs_Coluna_Sel Then
	This.Post SetColumn(ivs_ddw_MultiSelecao_Coluna)
	This.Modify(lvs_Coluna_Sel+".Visible='0'")
End If

// Ao sair do campo da lista de itens, mostrar os selecionados
If ivs_ddw_MultiSelecao_Ultima_Coluna = ivs_ddw_MultiSelecao_Coluna Then
	// Coluna existe?
	If Long(This.Describe(lvs_Coluna_Sel+'.id')) > 0 Then
		This.SetItem(This.GetRow(), lvs_Coluna_Sel, of_ddw_MultiSelecao_Get(", "))
		This.Modify(lvs_Coluna_Sel+".Visible='1'")
	End If
End If
end subroutine
...
event rowfocuschanged;If ivb_Selecao_Linhas_padrao Then
	This.of_setrowselection("","")
ElseIf ivb_Selecao_Linhas Then
...
If ivb_ddw_MultiSelecao Then
	This.of_ddw_MultiSelecao_Focus(dwo.Name)
	ivs_ddw_MultiSelecao_Ultima_Coluna = dwo.Name
End If
...
If ivb_ddw_MultiSelecao Then
	This.of_ddw_MultiSelecao_Focus(This.GetColumnName())
End If
end event
