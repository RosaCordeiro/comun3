HA$PBExportHeader$dc_w_login_sistema_wms.srw
forward
global type dc_w_login_sistema_wms from dc_w_response
end type
type cb_ok from commandbutton within dc_w_login_sistema_wms
end type
type cb_cancelar from commandbutton within dc_w_login_sistema_wms
end type
type dw_1 from dc_uo_dw_base within dc_w_login_sistema_wms
end type
end forward
...
global dc_w_login_sistema_wms dc_w_login_sistema_wms
...
public function boolean wf_valida_filial_cd (long al_cd_filial, ref string as_log)
public function boolean wf_permissao_usuario_filial (string as_usuario, long al_filial, ref string as_log)
end prototypes
...
Choose Case li_Retorno
	...
	Case 0
		...
		gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Digital n$$HEX1$$e300$$ENDHEX$$o localizada no cadastro', False, "B" )
	...
	Case 1
		gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Login com sucesso', True, "B" )
...
public function boolean wf_valida_filial_cd (long al_cd_filial, ref string as_log);String	ls_id_centro_distribuicao

select 
	id_centro_distribuicao
into
	:ls_id_centro_distribuicao
from
	filial
where
...
Choose case SQLCA.SQLCode
	Case 100
		as_log	= 'Filial n$$HEX1$$e300$$ENDHEX$$o encontrada.'
		Return False
	Case -1
		as_log	= 'Erro ao localizar filial. Erro: ' + SQLCA.SQLErrText
		Return False
	Case 0
		if ls_id_centro_distribuicao = 'S' then
			Return True
		else
			as_log = 'Filial do usu$$HEX1$$e100$$ENDHEX$$rio n$$HEX1$$e300$$ENDHEX$$o $$HEX1$$e900$$ENDHEX$$ um centro de distribui$$HEX2$$e700e300$$ENDHEX$$o, favor entrar em contato com seguran$$HEX1$$e700$$ENDHEX$$a de acesso.'
			Return False
		end if
end choose
...
public function boolean wf_permissao_usuario_filial (string as_usuario, long al_filial, ref string as_log);Long	ll_cd_filial


if gvo_aplicacao.ivo_seguranca.Id_Perfil_Master then
	return true
end if
...
from
	usuario
where 
	nr_matricula = :as_usuario;
	
Choose Case SQLCA.SQLCode
	Case -1
		as_log = 'Erro no SQL ao conferir permiss$$HEX1$$e300$$ENDHEX$$o para login em filial. Erro: ' + SQLCA.SQLErrText
		return false
	Case 100
		as_log = 'Registro do usu$$HEX1$$e100$$ENDHEX$$rio n$$HEX1$$e300$$ENDHEX$$o localizado para conferir permiss$$HEX1$$e300$$ENDHEX$$o para login em filial'
		return false
	Case 0
		if ll_cd_filial = al_filial then
			return true
		else
			as_log = 'Filial n$$HEX1$$e300$$ENDHEX$$o registrada no usu$$HEX1$$e100$$ENDHEX$$rio para acesso. Favor tratar com SEGURAN$$HEX1$$c700$$ENDHEX$$A DE ACESSO.'
			Return false
		end if
End Choose
...
on dc_w_login_sistema_wms.create
int iCurrent
call super::create
...
on dc_w_login_sistema_wms.destroy
call super::destroy
destroy(this.cb_ok)
...
type pb_help from dc_w_response`pb_help within dc_w_login_sistema_wms
end type
...
event clicked;String lvs_De_Senha, ls_nr_matricula
String lvs_Token
String ls_Senha_Expirada = 'N', ls_log
Long	ll_cd_filial, ll_width, ll_heigth, ll_x, ll_y
Integer lvi_Retorno
...
if IsNull(ll_cd_filial) or ll_cd_filial <= 0 then
	MessageBox('Aten$$HEX2$$e700e300$$ENDHEX$$o', 'Favor selecionar uma filial v$$HEX1$$e100$$ENDHEX$$lida.', StopSign!)
	dw_1.SetFocus()
	dw_1.SetColumn("cd_filial")
	Return -1
else	
	gl_cd_filial = ll_cd_filial
	gvo_Aplicacao.ivs_dado_extra_microhelp = 'Filial: ' + String(gl_cd_filial)
end if

OpenSheet(dc_w_principal_fundo, ParentWindow())

ll_width = ParentWindow().WorkSpaceWidth()
ll_heigth = ParentWindow().WorkSpaceHeight()

dc_w_principal_fundo.st_descricao_cd.Text = 'Filial: ' + String(gl_cd_filial)

dc_w_principal_fundo.Width = ll_width
dc_w_principal_fundo.Height = ll_heigth
dc_w_principal_fundo.x = 0
dc_w_principal_fundo.y = 0

If lvs_De_Senha <> "" Then
	//Busca novamente o usu$$HEX1$$e100$$ENDHEX$$rio para garantir o prefil via filial (usuario_sistema_filial)
	gvo_Aplicacao.ivo_Seguranca.of_localiza_usuario(ls_nr_matricula)
	...
	If lvs_De_Senha <> gvo_Aplicacao.ivo_Seguranca.De_Senha Then
		...
		If IsNumber(lvs_Token) Then
			If gvo_aplicacao.ivo_seguranca.of_valida_token_usuario( lvs_Token, gvo_Aplicacao.ivo_Seguranca.Nr_Matricula) Then
				...
				Else
					...
					gvo_Aplicacao.ivo_Seguranca.ivb_Login_Sucesso = True
					gvo_Aplicacao.of_SetMicroHelp()
					gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Login com sucesso', True, "D" )		
					Close(Parent)		
				End If	
			End If
		End If
		...
		If Not lvb_Alt_Senha Then
			...
			ivi_Tentativas = gvo_Aplicacao.ivo_Seguranca.Of_Get_Tentativas_Login(gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 3600) + 1
			...
			If ivi_Tentativas < gvo_Aplicacao.ivo_Seguranca.Limite_Tentativa_Login Then
				...
				gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Senha incorreta', False, "D" )
			...
			Else
				//Grava a tentativa de login no banco de dados
				gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Limite de tentativas esgotado', False, "D" )
	...
	Else
		If (gvo_Aplicacao.ivo_Seguranca.De_Senha = ivo_encript.of_encripta(gvo_Aplicacao.ivo_Seguranca.Nr_Matricula) or &
			...
			gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Altera$$HEX2$$e700e300$$ENDHEX$$o de senha', False, "A" )
		End If
		...
		If IsValid( dc_w_login_sistema_wms ) Then
			 Close( dc_w_login_sistema_wms )
		End If
	End If
Else
	gvo_Aplicacao.ivo_Seguranca.of_Registra_Tentativa_Login( gvo_Aplicacao.ivo_Seguranca.Nr_Matricula, 'Senha n$$HEX1$$e300$$ENDHEX$$o informada', False, "D" )
...
integer width = 1865
integer height = 488
string dataobject = "dc_dw_login_sistema_wms"
end type

event itemchanged;call super::itemchanged;String 	lvs_Nr_Matricula, ls_log, ls_null
Long		ll_cd_filial_usuario, ll_null
...
Choose Case dwo.Name
	Case "nr_matricula"
		...
		//Verifica se permite trocar filial ou n$$HEX1$$e300$$ENDHEX$$o
		ll_cd_filial_usuario = gvo_Aplicacao.il_cd_filial
		
		if /*gvo_Aplicacao.ivo_Seguranca.id_Perfil_Master or*/ ll_cd_filial_usuario = 9999 then
			gf_desbloqueia_campo_dw(dw_1, 'cd_filial')
		else
			if wf_valida_filial_cd(ll_cd_filial_usuario, REF ls_log) then
				dw_1.SetItem(dw_1.GetRow(), 'cd_filial', ll_cd_filial_usuario)
			else
				MessageBox('Aten$$HEX2$$e700e300$$ENDHEX$$o', ls_log, StopSign!)
				cb_Ok.Enabled = False
				dw_1.SetItem(1, 'nr_matricula', ls_null)
				dw_1.SetItem(1, 'Nm_Usuario', ls_null)
				dw_1.SetItem(1, 'cd_filial', ll_null)
				dw_1.SetColumn('nr_matricula')
				Return 1
			end if
			
			gf_bloqueia_campo_dw(dw_1, 'cd_filial')
		end if
		...
		If ( Not gvo_Aplicacao.ivb_Usa_Biometria ) Or ( gvo_Aplicacao.ivo_Seguranca.Id_Isento_Biometria = 'S' ) &
