...
Subroutine GlobalMemoryStatusEx(Ref str_memorystatus astr_memorystatus) Library "KERNEL32.DLL"; // Informa$$HEX2$$e700f500$$ENDHEX$$es sobre mem$$HEX1$$f300$$ENDHEX$$ria
...
If Not FileExists(ivs_Arquivo_INI) Then
	MessageBox("Aten$$HEX2$$e700e300$$ENDHEX$$o", "Arquivo de inicializa$$HEX2$$e700e300$$ENDHEX$$o '" + ivs_Arquivo_INI + "' n$$HEX1$$e300$$ENDHEX$$o localizado.", StopSign!)
	This.Event ue_Exit()
End If
...
public function boolean of_abre_log ();This.ivi_LOG = FileOpen(This.ivs_Arquivo_LOG, LineMode!, Write!, Shared!, Append!)
...
public function boolean of_grava_log (string ps_mensagem);Integer lvi_Status
...
public function string of_getfromini (string ps_secao, string ps_chave);String lvs_Valor

lvs_Valor = ProfileString(ivs_Arquivo_INI, ps_Secao, ps_Chave, "")

If Trim(lvs_Valor) = "" Then	
...
If lvs_DataBase = "" Then
	lvs_DataBase	= ProfileString( lvs_Ini, "DataBase", "DataBase", "" )
	lvs_DataSource	= ProfileString( lvs_Ini, "DataBase", "DataSource", "" )
	lvs_Selecao_BD	= ProfileString( lvs_Ini, "DataBase", "Seleciona_BD", "N" )
End If

If (lvs_Selecao_BD='S')and(ivo_Seguranca.Cd_Sistema <> 'VV') Then
...
If ivo_Seguranca.Cd_Sistema = 'RL' Then	
	ls_Seleciona_Odbc	= ProfileString( lvs_Ini, gvo_Aplicacao.ivo_Seguranca.Cd_Sistema, "Seleciona_Odbc", "N" )

	If ls_Seleciona_Odbc = "S" Then
		OpenWithParm( w_selecao_filial_odbc, '' )
...
public function string of_get_system_directory ();string ls_PathName,ls_Command
...
public subroutine of_registra_versao_bd ();Decimal ldc_Espaco_Livre_C
...
If RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\", ls_Chave, RegString!, ls_Windows) = -1 Then
	RegistryGet("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\", ls_Chave, RegString!, ls_Windows)	
End If

ls_Windows = Upper(ls_Windows)

If Not gf_filiais_parametro( Ref  ll_Filial, Ref ll_Filial_Matriz ) Then Return

If RightA( '0000' + String( ll_Filial ), 4 ) <> LeftA( ls_Pdv, 4 ) Then Return

If ll_Filial = ll_Filial_Matriz Then Return
...
l_str.ul_length = 64
GlobalMemoryStatusEx(Ref l_str)

// Divis$$HEX1$$e300$$ENDHEX$$o feita por 1024 e novamente por 1024 por estar em bytes
ls_Windows += " - MEM. RAM: " + String( Truncate( l_str.ul_TotalPhys / 1048576, 0 ) , "####" ) + "M"
ls_Windows = RightA( ls_Windows, 40 )
...
Select nr_versao
  Into :ls_Achou
  From sistema_pdv
 Where cd_sistema	= :ivo_Seguranca.Cd_Sistema
	And nm_pdv			= :ls_Pdv
 Using SqlCa; 
...
Choose Case Sqlca.SqlCode
	Case 100
		Insert
		 ...
		 Values(:ivo_Seguranca.Cd_Sistema,
				  :ls_Pdv, 
				  :ivs_Versao,
				  getdate(),
				  getdate(),
	...
	Case 0
		...
		If ls_Achou <> ivs_Versao Then
		
			Update sistema_pdv
				Set nr_versao					= :ivs_Versao, 
					 dh_versao		   			= getdate(),
		...
		If SqlCa.SqlCode = -1 Then
			SqlCa.of_RollBack()
			SqlCa.of_MsgDbError( "Atualiza$$HEX2$$e700e300$$ENDHEX$$o da tabela sistema_pdv - gvo_Aplicacao.of_Registra_Versao_BD()" )
		End If
		
	Case -1
		SqlCa.of_RollBack( )
		SqlCa.of_MsgDbError( "gvo_Aplicacao.of_Registra_Versao_BD( )" )
		Return
End Choose
...
public function boolean of_is64bits ();Boolean wow64 = False
...
public subroutine of_atualiza_vv ();String ls_Versao
...
RegistryGet( THIS.REGEDIT_VERSOES_SISTEMAS_CLAMED, 'VV', RegString!, ls_Versao_Registro )
...
Try
	...
	If String( ll_Filial_Parametro, "0000" ) = LeftA( ls_PDV, 4 ) Then	
		...
		If SqlCa.SqlCode = -1 Then
			SqlCa.of_RollBack()
			SqlCa.of_MsgDbError( "gvo_aplicacao.of_atualiza_vv()" )
			Return
		End If
		...
		If SqlCa.SqlCode = -1 Then
			SqlCa.of_RollBack()
			SqlCa.of_MsgDbError( "application.af_atualiza_vv()" )
			Return
		End If
	...
	FileDelete( ls_Caminho_Sistema_Executando + ls_Zip )
	RegistrySet( THIS.REGEDIT_VERSOES_SISTEMAS_CLAMED, 'VV', RegString!, ls_Versao )
...
Try
	...
	If MidA( This.is_ComputerName, 1, 1 ) = '0' Then
			...
			Case 'CL', 'RL'
				If ls_Mac_Parametro <> ls_Mac_Computador Then
					If lo_Parametro_PDV.of_Limpa_Informacoes_PDV( ) Then
						...
						gf_ge202_envia_email_log( 63, 'Computador trocado', '' )						
					End If
				End If
