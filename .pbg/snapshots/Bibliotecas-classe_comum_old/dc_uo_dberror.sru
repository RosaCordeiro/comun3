...
public subroutine of_grava_log (integer pi_arquivo, string ps_mensagem);Boolean lb_Abriu_Log = False
...
If pi_Arquivo > 0 Then
	lvi_Status = FileWrite( pi_Arquivo, lvs_Mensagem )

	If lvi_Status <> LenA( lvs_Mensagem ) Then	
		MessageBox( "Aten$$HEX2$$e700e300$$ENDHEX$$o", "Erro na grava$$HEX2$$e700e300$$ENDHEX$$o do registro no log. Mensagem: " + ps_Mensagem + "~r~rlvi_Status: " + String( lvi_Status ) + " | LenA(lvs_Mensagem): " + String( LenA(lvs_Mensagem) ) + '~rdc_uo_dberror.of_grava_log( integer, string ) ', StopSign! )
		Halt Close
	End If
End If
end subroutine
...
public subroutine of_erro_saida (string ps_titulo, string ps_texto);If gvo_Aplicacao.ivo_Seguranca.Cd_Sistema = 'TL' Then This.of_SEt_erro_saida_padrao( 'LOG' )
