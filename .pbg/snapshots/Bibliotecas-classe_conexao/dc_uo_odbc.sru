...
If RegistrySet( ls_Chave, "Driver", RegString!, "C:\Program Files (x86)\MySQL\Connector ODBC 5.1\myodbc5.dll" ) = 1 Then
	//If RegistrySet( ls_Chave, "UID", RegString!, "" ) = 1 Then // Removido em 21/12/2016, caso apresente problema, informar Fernando Cambiaghi
		//If RegistrySet( ls_Chave, "PWD", RegString!, "" ) = 1 Then // Removido em 21/12/2016, caso apresente problema, informar Fernando Cambiaghi
			If RegistrySet(ls_Chave, "DATABASE", RegString!, "softdesk" ) = 1 Then
				If RegistrySet( ls_Chave, "SERVER", RegString!, "helpdesk.clamed.com.br" ) = 1 Then
					If RegistrySet( ls_Chave, "PORT", RegString!, "3306") = 1 Then
						If RegistrySet( ls_Chave, "DESCRIPTION", RegString!, "ServiceDesk") = 1 Then
							If RegistrySet( This.CHAVE_ODBC_INI + "ODBC Data Sources", "servicedesk", RegString!, "MySQL ODBC 5.1 Driver" ) = 1 Then
								Return True			
							End If
						End If
					End If
...
SELECT  p.cd_filial, p.de_database, p.de_servername, p.de_ip, coalesce(p.id_pdv_java,'N'), coalesce(f.id_recebe_nf_transferencia, 'S')
  FROM parametro_odbc p
  	INNER JOIN filial f
...
Try
	...
	FETCH lcur_odbc INTO :ll_Filial, :ls_Database, :ls_ServerName, :ls_IP, :ls_Pdv_Java, :ls_Recebe_Transf ;
	...
	DO WHILE SQLCA.sqlcode = 0
		...
		If ls_Recebe_Transf = 'N' Then
			ls_Group = 'Inativa'
		Else
			If ls_Pdv_Java = 'S' Then
				ls_Group = 'Java'
			Else
				ls_Group = 'Legado'
			End If
		...
		If RegistrySet( "HKEY_CURRENT_USER\Software\pgAdmin III\Servers\" + String( li_Linha ), "Group", RegString!, ls_Group ) = -1 Then
			MessageBox( "Aten$$HEX2$$e700e300$$ENDHEX$$o", "Erro na grava$$HEX2$$e700e300$$ENDHEX$$o do registro. [HKEY_CURRENT_USER\Software\pgAdmin III\Servers\" + String( li_Linha ) + "], Group=" + ls_Group )
		End If
