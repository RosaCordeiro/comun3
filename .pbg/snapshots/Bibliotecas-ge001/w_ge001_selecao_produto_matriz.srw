HA$PBExportHeader$w_ge001_selecao_produto_matriz.srw
forward
global type w_ge001_selecao_produto_matriz from dc_w_selecao_generica
end type
end forward
...
global w_ge001_selecao_produto_matriz w_ge001_selecao_produto_matriz
...
on w_ge001_selecao_produto_matriz.create
call super::create
end on

on w_ge001_selecao_produto_matriz.destroy
call super::destroy
end on
...
type pb_help from dc_w_selecao_generica`pb_help within w_ge001_selecao_produto_matriz
end type

type gb_2 from dc_w_selecao_generica`gb_2 within w_ge001_selecao_produto_matriz
integer x = 18
integer y = 384
integer width = 3497
integer height = 1272
...
type gb_1 from dc_w_selecao_generica`gb_1 within w_ge001_selecao_produto_matriz
integer x = 18
integer y = 8
integer width = 3063
integer height = 368
...
type dw_1 from dc_w_selecao_generica`dw_1 within w_ge001_selecao_produto_matriz
integer x = 37
integer y = 72
integer width = 3031
integer height = 288
...
type dw_2 from dc_w_selecao_generica`dw_2 within w_ge001_selecao_produto_matriz
integer x = 46
integer y = 432
integer width = 3438
integer height = 1200
string dataobject = "dw_ge001_lista_matriz"
...
If Not IsNull(lvs_Descricao) and Trim(lvs_Descricao) <> "" Then
	...
	*/
	...
	If LeftA( lvs_Descricao, 3 ) = '%ZZ' Then
		This.of_AppendWhere("( de_produto like '%" + MidA( lvs_Descricao, 4 ) + "%' Or de_produto like '%ZZ" + MidA( lvs_Descricao, 4 ) + "%' )")
	Else
		This.of_AppendWhere("( de_produto like '" + lvs_Descricao_Busca + "%' Or de_produto like 'ZZ" + lvs_Descricao_Busca + "%' )")		
	End If
	/***/
End If
...
If Not IsNull(lvs_Barras) and Trim(lvs_Barras) <> "" Then	
	...
	This.of_AppendWhere("cd_produto in (select cd_produto from codigo_barras_produto where de_codigo_barras = '" + lvs_GTIN + "')")
End If
...
If Not IsNull(lvs_Sap) and Trim(lvs_Sap) <> "" Then
	...
	if mid(lvs_Sap,1,1) = '%' Then
		This.of_AppendWhere("cd_produto_sap like '" + lvs_Sap + "'")	
	else
		This.of_AppendWhere("cd_produto_sap = '" + lvs_Sap + "'")
	end if
...
event dw_2::constructor;call super::constructor;wf_Set_Somente_Consulta()
end event

type cb_selecionar from dc_w_selecao_generica`cb_selecionar within w_ge001_selecao_produto_matriz
integer x = 2738
integer y = 1688
end type
...
type cb_cancelar from dc_w_selecao_generica`cb_cancelar within w_ge001_selecao_produto_matriz
integer x = 3136
integer y = 1688
end type
...
type cb_pesquisar from dc_w_selecao_generica`cb_pesquisar within w_ge001_selecao_produto_matriz
integer x = 2144
integer y = 1688
end type

type st_mensagem from dc_w_selecao_generica`st_mensagem within w_ge001_selecao_produto_matriz
integer x = 37
integer y = 1700
integer width = 2021
long backcolor = 80269524
