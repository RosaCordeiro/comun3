HA$PBExportHeader$dc_uo_sftp.sru
forward
global type dc_uo_sftp from nonvisualobject
end type
end forward

global type dc_uo_sftp from nonvisualobject
end type
global dc_uo_sftp dc_uo_sftp
...
public function boolean of_enviar_arquivo_api (string ps_dir_file, string ps_servidor, string ps_usuario, string ps_senha, long al_porta, string ps_dir, string ps_filename, string ps_tipo_envio, string ps_nome_arquivo, ref string ps_log)
end prototypes
...
public function boolean of_enviar_arquivo_api (string ps_dir_file, string ps_servidor, string ps_usuario, string ps_senha, long al_porta, string ps_dir, string ps_filename, string ps_tipo_envio, string ps_nome_arquivo, ref string ps_log);Blob 			lblob_data, lblob_file, lblob_files[]
...
Try
	...
	Choose Case ps_tipo_envio
			
		Case 'C'
			llb_temp.DirList( ps_dir_file + ps_nome_arquivo + "*.*", 0)	
			
		Case	'Z'
			llb_temp.DirList( ps_dir_file + '*.'+lower('zip') , 0)
		
		Case 'P'
			llb_temp.DirList( ps_dir_file + '*.'+lower(is_format) , 0)
			
		Case else
			llb_temp.DirList( ps_dir_file + '*.'+lower('ped') , 0)
...
on dc_uo_sftp.create
call super::create
TriggerEvent( this, "constructor" )
end on

on dc_uo_sftp.destroy
