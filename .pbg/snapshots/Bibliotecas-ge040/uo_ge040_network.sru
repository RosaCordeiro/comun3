HA$PBExportHeader$uo_ge040_network.sru
$PBExportComments$Userobject to perform network Ping
forward
global type uo_ge040_network from nonvisualobject
end type
type icmp_echo_reply from structure within uo_ge040_network
end type
type large_integer from structure within uo_ge040_network
end type
type hostent from structure within uo_ge040_network
end type
type wsadata from structure within uo_ge040_network
end type
end forward
...
global uo_ge040_network uo_ge040_network
...
public function boolean of_ping (string as_ipaddress, string as_echomsg, ref string as_erro)
end prototypes
...
Return of_Ping(as_ipaddress, ls_echomsg, ref ls_echomsg )
...
If lul_address > 0 Then
	...
	Else
		If lstr_reply.Status = 0 Then
			...
			End If
		Else
			as_Erro = of_FormatMessage(lstr_reply.Status)
		End If
	End If
Else
	as_Erro = "The given IP Address is invalid!"
End If
...
on uo_ge040_network.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_ge040_network.destroy
