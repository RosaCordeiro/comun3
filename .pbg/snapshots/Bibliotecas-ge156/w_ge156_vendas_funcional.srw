HA$PBExportHeader$w_ge156_vendas_funcional.srw
forward
global type w_ge156_vendas_funcional from dc_w_selecao_lista_relatorio
end type
end forward

global type w_ge156_vendas_funcional from dc_w_selecao_lista_relatorio
string tag = "w_ge156_vendas_funcional"
integer width = 4229
integer height = 1880
string title = "GE156 - Vendas Funcional por Produto"
end type
global w_ge156_vendas_funcional w_ge156_vendas_funcional
...
public subroutine wf_insere_default ();/* Conv$$HEX1$$ea00$$ENDHEX$$nio Funcional */
idwc_Child  = dw_1.of_InsertRow_DDDW("cd_convenio" )			

idwc_Child.SetItem(1, "cd_conveniado", 'TD'	)
...
idwc_Child.SetItem(1, "cd_convenio", 52349)
idwc_Child.SetFilter('cd_convenio=52349')
...
on w_ge156_vendas_funcional.create
call super::create
end on

on w_ge156_vendas_funcional.destroy
call super::destroy
...
ivds_desconto.of_changedataobject('ds_ge156_desconto_produto')

ivds_devolucao = Create dc_uo_ds_base
ivds_devolucao.of_changedataobject('ds_ge156_devolucao')

ivds_programa = Create dc_uo_ds_base
ivds_programa.of_changedataobject('ds_ge156_programa_pbm')
...
type dw_visual from dc_w_selecao_lista_relatorio`dw_visual within w_ge156_vendas_funcional
end type

type gb_aux_visual from dc_w_selecao_lista_relatorio`gb_aux_visual within w_ge156_vendas_funcional
end type

type gb_2 from dc_w_selecao_lista_relatorio`gb_2 within w_ge156_vendas_funcional
integer y = 424
integer width = 4128
integer height = 1264
end type

type gb_1 from dc_w_selecao_lista_relatorio`gb_1 within w_ge156_vendas_funcional
integer width = 4128
integer height = 412
end type

type dw_1 from dc_w_selecao_lista_relatorio`dw_1 within w_ge156_vendas_funcional
integer width = 4073
integer height = 312
string dataobject = "dw_ge156_selecao"
end type
...
type dw_2 from dc_w_selecao_lista_relatorio`dw_2 within w_ge156_vendas_funcional
integer x = 64
integer y = 492
integer width = 4059
integer height = 1156
string dataobject = "dw_ge156_lista"
...
If lvs_Prod_PBM = 'S' Then
	If lvdt_Inicio < Date('01/05/2015') Then
...
If lvs_Manipulado = 'N' Then
	This.of_appendwhere('coalesce(x.cd_produto,0)<>684431')
	ivds_devolucao.of_appendwhere('coalesce(inf.cd_produto,0)<>684431')
End If
	...
	If lvs_Devolucao = 'S' Then
		...
		For lvl_Linha = 1 To lvl_Linhas
			...
			If (lvl_PBM > 0)and(Not IsNull(lvl_PBM)) Then
				If Trim(lvs_convenio) <> '' And not IsNull(lvs_convenio) Then 
				...
				Else
					lvl_Find =This.Find("cd_filial="+String(lvl_Filial)+ &
										" and cd_produto="+String(lvl_Produto)+" and cd_pbm="+String(lvl_PBM),1,This.RowCount())					
				End If
			Else
				If Trim(lvs_convenio) <> '' And not IsNull(lvs_convenio) Then 
...
type dw_3 from dc_w_selecao_lista_relatorio`dw_3 within w_ge156_vendas_funcional
integer x = 3593
integer y = 28
integer height = 220
end type
