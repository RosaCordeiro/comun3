...
Else
	...
	If ib_messagebox Then
		...
		choose case ai_msglevel
			case iINFO
				gvo_Aplicacao.of_Grava_Log("Winsock Info Msg :: " + as_msgtext)
			case iDEBUG
				gvo_Aplicacao.of_Grava_Log("Winsock Debug Msg :: " + as_msgtext)
			case iERROR
				gvo_Aplicacao.of_Grava_Log("Winsock Error Msg :: " + as_msgtext)
			case else
				gvo_Aplicacao.of_Grava_Log("Winsock Msg Level " + String(ai_msglevel) + " :: " + as_msgtext)
		end choose
	End If
End If
end subroutine
...
If lul_ptr > 0 Then
	...
	If connect_ws(lui_socket, lstr_sockaddr, MAXGETSOCKADDRSTRUCT) = -1 Then
		ls_errmsg = of_GetLastError()
		this.of_LogError(iERROR, &
			"Winsock Error in of_Connect: " + ls_errmsg)
		lui_socket = 0
	End if
Else
	ls_errmsg = of_GetLastError()
...
// ----------  --------		-----------------------------------------------------
// 12/29/2006	RolandS		Initial coding
// 07/09/2009	RolandS		Added FD_READ to WSAASyncSelect call
// -----------------------------------------------------------------------------
...
If lul_ptr > 0 Then
	...
	Else
		...
		If WSAASyncSelect(lui_socket, al_handle, WM_USER + (ai_event - 1), ll_wsevent) = -1 Then
			ls_errmsg = of_GetLastError()
			this.of_LogError(iERROR, &
				"Winsock Error in of_Listen: " + ls_errmsg)
			lui_socket = 0
