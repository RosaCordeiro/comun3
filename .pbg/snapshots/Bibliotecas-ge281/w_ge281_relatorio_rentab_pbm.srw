HA$PBExportHeader$w_ge281_relatorio_rentab_pbm.srw
forward
global type w_ge281_relatorio_rentab_pbm from dc_w_selecao_lista_relatorio
end type
type pb_info from picturebutton within w_ge281_relatorio_rentab_pbm
end type
end forward
...
integer width = 4489
integer height = 1888
string title = "GE281 - Relat$$HEX1$$f300$$ENDHEX$$rio de Rentabilidade PBM"
long backcolor = 80269524
pb_info pb_info
end type
global w_ge281_relatorio_rentab_pbm w_ge281_relatorio_rentab_pbm
...
Try
	...
	If Not lvds_1.Of_ChangeDataObject("ds_ge281_venda") Then	Return	
	If Not lvds_2.Of_ChangeDataObject("ds_ge281_devolucao") Then Return
	...
	If Not IsNull(lvl_Produto) Then
		lvds_1.of_appendwhere_subquery("i.cd_produto = " + String(lvl_Produto),6)
		lvds_2.of_AppendWhere("i.cd_produto = " + String(lvl_Produto))
	End If
	
	If Not IsNull(lvl_Filial) Then
		lvds_1.of_appendwhere_subquery("vp.cd_filial = " + String(lvl_Filial),6)
	...
	If lvs_Id_Grupo <> '0' Then
		lvds_1.of_appendwhere_subquery("substring(g.cd_subcategoria,1,1) = '" + lvs_Id_Grupo + "'",6)
	...
	If lvs_Programa<>'' Then
		Choose Case lvl_Convenio
			Case 52575, 53724, 53725 //VIDALINK
				lvds_1.of_appendwhere_subquery("vp.nr_comprovante_venda='"+lvs_Programa+"'",6)
				lvds_2.of_AppendWhere("vp.nr_comprovante_venda='"+lvs_Programa+"'")
			Case Else 
				lvds_1.of_appendwhere_subquery("vp.cd_convenio_pbm='"+lvs_Programa+"'",6)
	...
	If (lvs_PBM = 'S')or(lvl_PBM>0) Then
		...
		If adt_Inicio >= Date('01/05/2015') Then
			lvds_1.of_appendwhere_subquery("vpp.id_pbm='S'",6)		
			lvds_2.of_AppendWhere("vpp.id_pbm='S'")		
		End If		
		If (adt_Inicio <= Date('30/04/2015'))or(lvl_PBM>0) Then
			lvds_1.of_appendwhere_subquery("exists (select pb1.cd_convenio, pb1.cd_pbm, p1.cd_produto "	+ &
													"from pbm pb1 " 													+ &
													...
													"group by pb1.cd_convenio, pb1.cd_pbm, p1.cd_produto) ",6)
	...
	//Se o convenio for Vidalink Diversos pode ter que deduzir devolu$$HEX2$$e700f500$$ENDHEX$$es do convenio 52575 (c$$HEX1$$f300$$ENDHEX$$digo antigo)
	If adt_Inicio >= Date('01/11/2016') Then
		//Vidalink Diversos
		If (lvl_Convenio = 53725) Then
			lvds_2.Of_AppendWhere("vp.cd_convenio in ("+String(lvl_Convenio)+",52575)")
			lvds_2.Of_AppendWhere("(((vp.cd_convenio = 52575) and (vp.nr_comprovante_venda<>'AVFARPOP')) or (vp.cd_convenio <> 52575))")
		//Farm. Popular 
		ElseIf (lvl_Convenio = 52575) Then
			lvds_2.Of_AppendWhere("(vp.cd_convenio = 52575) and (vp.nr_comprovante_venda='AVFARPOP')")
		Else		
	...
	Open(w_aguarde)
	w_aguarde.Title = 'Recuperando as vendas...'
	
	//lvs_Aux = lvds_1.GetSQLSelect()
	lvds_1.Retrieve(adt_Inicio, adt_Termino,lvl_Convenio)
	
	w_aguarde.Title = 'Recuperando as devolu$$HEX2$$e700f500$$ENDHEX$$es...'
	...
	w_aguarde.uo_Progress.Of_SetMax(lvl_Linhas)
	w_aguarde.Title = 'Processando as devolu$$HEX2$$e700f500$$ENDHEX$$es...'
...
on w_ge281_relatorio_rentab_pbm.create
int iCurrent
call super::create
...
on w_ge281_relatorio_rentab_pbm.destroy
call super::destroy
...
type dw_visual from dc_w_selecao_lista_relatorio`dw_visual within w_ge281_relatorio_rentab_pbm
end type

type gb_aux_visual from dc_w_selecao_lista_relatorio`gb_aux_visual within w_ge281_relatorio_rentab_pbm
end type

type gb_2 from dc_w_selecao_lista_relatorio`gb_2 within w_ge281_relatorio_rentab_pbm
integer x = 18
integer y = 416
integer width = 4393
integer height = 1268
...
type gb_1 from dc_w_selecao_lista_relatorio`gb_1 within w_ge281_relatorio_rentab_pbm
integer x = 18
integer y = 8
integer width = 4178
integer height = 388
...
type dw_1 from dc_w_selecao_lista_relatorio`dw_1 within w_ge281_relatorio_rentab_pbm
integer x = 46
integer y = 72
integer width = 4137
integer height = 300
string dataobject = "dw_ge281_selecao"
end type
...
type dw_2 from dc_w_selecao_lista_relatorio`dw_2 within w_ge281_relatorio_rentab_pbm
integer x = 50
integer y = 492
integer width = 4311
integer height = 1136
string dataobject = "dw_ge281_lista"
...
type dw_3 from dc_w_selecao_lista_relatorio`dw_3 within w_ge281_relatorio_rentab_pbm
integer x = 1376
integer y = 36
integer width = 375
integer height = 116
string dataobject = "dw_ge281_relatorio"
end type
