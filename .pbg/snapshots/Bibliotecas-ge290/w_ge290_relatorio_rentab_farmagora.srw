HA$PBExportHeader$w_ge290_relatorio_rentab_farmagora.srw
forward
global type w_ge290_relatorio_rentab_farmagora from dc_w_selecao_lista_relatorio
end type
type pb_info from picturebutton within w_ge290_relatorio_rentab_farmagora
end type
end forward

global type w_ge290_relatorio_rentab_farmagora from dc_w_selecao_lista_relatorio
string tag = "w_ge290_relatorio_rentab_farmagora"
integer width = 4261
integer height = 1888
string title = "GE290 - Relat$$HEX1$$f300$$ENDHEX$$rio de Rentabilidade do Ecommerce"
long backcolor = 80269524
pb_info pb_info
end type
global w_ge290_relatorio_rentab_farmagora w_ge290_relatorio_rentab_farmagora
...
Try
	...
	If Not lvds_1.Of_ChangeDataObject("ds_ge290_venda") Then
		Destroy(lvds_1)
		Return
	End If
	...
	If Not lvds_2.Of_ChangeDataObject("ds_ge290_devolucao") Then
		Destroy(lvds_1)
		Destroy(lvds_2)
		Return
	End If
	...
	lvs_Rede			= dw_1.Object.id_rede_ecommerce	[1]
	lvs_Tipo			= dw_1.Object.id_tipo_ecommerce	[1]
	lvl_Entrega		= dw_1.Object.id_modo_entrega		[1]
	lvs_id_tipo_pedido = dw_1.Object.id_tipo_pedido	[1]
	
	if not isnull(lvs_id_tipo_pedido) and lvs_id_tipo_pedido <> 'TOD' and lvs_id_tipo_pedido <> '' then
		lvds_1.of_AppendWhere("pea.id_tipo_pedido = '" + lvs_id_tipo_pedido + "'" )
		lvds_2.of_AppendWhere("pea.id_tipo_pedido = '" + lvs_id_tipo_pedido + "'" )
	ENd if
	...
	If lvs_Frete = 'N' Then
		lvds_1.of_AppendWhere("i.cd_produto not in (712055)")
		lvds_2.of_AppendWhere("i.cd_produto not in (712055)")
	End If
	
	If lvs_Rede <> 'TD' Then
		lvds_1.of_AppendWhere("coalesce(pea.id_rede_ecommerce,f.id_rede_filial)='"+lvs_Rede+"'")
		lvds_2.of_AppendWhere("coalesce(pea.id_rede_ecommerce,f.id_rede_filial)='"+lvs_Rede+"'")
	End If
	
	Choose Case lvs_Tipo
		Case "AP" //Aplicativo mobile
			lvds_1.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='2'") //Todas as vendas VTEX, depois ajustar o filtro quando for poss$$HEX1$$ed00$$ENDHEX$$vel identificar
			lvds_2.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='2'") //Todas as vendas VTEX, depois ajustar o filtro quando for poss$$HEX1$$ed00$$ENDHEX$$vel identificar
			lvds_1.of_AppendWhere("pe.de_canal_compra_vtex in ('app_android', 'app_ios', 'app_android_n', 'app_ios_n')") 
			lvds_2.of_AppendWhere("pe.de_canal_compra_vtex in ('app_android', 'app_ios', 'app_android_n', 'app_ios_n')") 
			
		Case "IF" //Ifood
			lvds_1.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='3'") //Todas as vendas VTEX, depois ajustar o filtro quando for poss$$HEX1$$ed00$$ENDHEX$$vel identificar
			lvds_2.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='3'") //Todas as vendas VTEX, depois ajustar o filtro quando for poss$$HEX1$$ed00$$ENDHEX$$vel identificar
			
							
		Case "SI" //Sites (todas as plataformas
			lvds_1.of_AppendWhere("coalesce(pe.id_ecommerce,'1') not in ('3','5')") //Todas as vendas VTEX, exceto iFood, depois tem de excluir as vendas APP
			lvds_2.of_AppendWhere("coalesce(pe.id_ecommerce,'1') not in ('3','5')") //Todas as vendas VTEX, exceto iFood, depois tem de excluir as vendas APP
			lvds_1.of_AppendWhere("coalesce(pe.de_canal_compra_vtex,'') not in ('app_android', 'app_ios', 'app_android_n', 'app_ios_n')") 
			lvds_2.of_AppendWhere("coalesce(pe.de_canal_compra_vtex,'') not in ('app_android', 'app_ios', 'app_android_n', 'app_ios_n')") 
		
			Case "CR" //Consulta Rem$$HEX1$$e900$$ENDHEX$$dio
			lvds_1.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='5'") //Todas as vendas da plataforma do Consulta Rem$$HEX1$$e900$$ENDHEX$$dio
			lvds_2.of_AppendWhere("coalesce(pe.id_ecommerce,'1')='5'") //Todas as devolu$$HEX2$$e700f500$$ENDHEX$$es da plataforma do Consulta Rem$$HEX1$$e900$$ENDHEX$$rio
	End Choose
	
	If lvl_entrega > 0 and not isnull(lvl_entrega) Then
		lvds_1.of_AppendWhere("pe.cd_tipo_entrega = " + string(lvl_entrega) )
		lvds_2.of_AppendWhere("pe.cd_tipo_entrega = " + string(lvl_entrega) )
	End If
	
	Open(w_aguarde)
	w_aguarde.Title = 'Recuperando informa$$HEX2$$e700f500$$ENDHEX$$es das vendas...'
	Yield()
	
	lvds_1.Retrieve(adt_Inicio, adt_Termino)
	w_aguarde.Title = 'Recuperando informa$$HEX2$$e700f500$$ENDHEX$$es das devolu$$HEX2$$e700f500$$ENDHEX$$es de vendas...'
	Yield()
	
	lvl_Linhas = lvds_2.Retrieve(adt_Inicio, adt_Termino)
	w_aguarde.Title = 'Deduzindo devolu$$HEX2$$e700f500$$ENDHEX$$es das vendas...'
	Yield()
	...
	For lvl_Linha = 1 To lvl_Linhas
		...
		lvl_Find = lvds_1.Find("cd_filial="+String(lvl_Filial)+" and cd_produto="+String(lvl_Produto),1,lvds_1.RowCount())
	...
	w_aguarde.Title = 'Processando informa$$HEX2$$e700f500$$ENDHEX$$es...'
	w_aguarde.uo_Progress.Of_SetMax(lvl_Linhas)
	Yield()
	...
	For lvl_Linha = 1 To lvl_Linhas
		...
		lvs_Tributa				= lvds_1.Object.cd_tributacao_icms	[lvl_Linha]		
...
/* Modo de Entrega */
ldwc_Child  = dw_1.of_InsertRow_DDDW("id_modo_entrega" )			

//ldwc_Child.SetItem(1, "cd_modo_entrega", "")
//ldwc_Child.SetItem(1, "de_modo_entrega", "TODAS")
//
dw_1.Object.id_modo_entrega[1] =0
end subroutine
...
on w_ge290_relatorio_rentab_farmagora.create
int iCurrent
call super::create
...
on w_ge290_relatorio_rentab_farmagora.destroy
call super::destroy
...
event ue_preopen;call super::ue_preopen;MaxWidth	= 5500
MaxHeight	= 9999
end event
...
type dw_visual from dc_w_selecao_lista_relatorio`dw_visual within w_ge290_relatorio_rentab_farmagora
end type

type gb_aux_visual from dc_w_selecao_lista_relatorio`gb_aux_visual within w_ge290_relatorio_rentab_farmagora
end type

type gb_2 from dc_w_selecao_lista_relatorio`gb_2 within w_ge290_relatorio_rentab_farmagora
integer x = 23
integer y = 536
integer width = 4178
integer height = 1160
...
type gb_1 from dc_w_selecao_lista_relatorio`gb_1 within w_ge290_relatorio_rentab_farmagora
integer x = 18
integer y = 8
integer width = 2994
integer height = 516
...
type dw_1 from dc_w_selecao_lista_relatorio`dw_1 within w_ge290_relatorio_rentab_farmagora
integer x = 46
integer y = 76
integer width = 2953
integer height = 428
string dataobject = "dw_ge290_selecao"
end type
...
event dw_1::itemchanged;call super::itemchanged;Choose Case dwo.Name
	...
	Case 'id_tipo_ecommerce'
		If Data = "IF" Then //IFOOD
			This.Object.id_modo_entrega	[1] = 0
		End If
End Choose
end event
...
type dw_2 from dc_w_selecao_lista_relatorio`dw_2 within w_ge290_relatorio_rentab_farmagora
integer x = 50
integer y = 584
integer width = 4096
integer height = 1052
string dataobject = "dw_ge290_lista"
...
lvs_Nome = {"C$$HEX1$$f300$$ENDHEX$$digo Produto",&
...
type dw_3 from dc_w_selecao_lista_relatorio`dw_3 within w_ge290_relatorio_rentab_farmagora
integer x = 3054
integer y = 40
integer width = 375
integer height = 116
string dataobject = "dw_ge290_relatorio"
end type
