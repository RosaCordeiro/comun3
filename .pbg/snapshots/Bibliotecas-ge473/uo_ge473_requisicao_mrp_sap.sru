...
//				as_Log = 'N$$HEX1$$fa00$$ENDHEX$$mero da requis$$HEX1$$e300$$ENDHEX$$o do item [' + String(ll_NR_Requisicao) + '] est$$HEX1$$e100$$ENDHEX$$ diferente do cabe$$HEX1$$e700$$ENDHEX$$alho ['  + String(il_nr_requisicao) + '], registro pai.'
			...
			ll_QT_Requisicao = Long(gf_Replace(lo_Comum.ids_lista_registros.Object.qt_requisicao[ll_Linha], '.', ',', 0))
...
try 
	...
	If Not lds.of_ChangeDataObject('ds_ge473_lista_controles', False) Then 
		gvo_aplicacao.of_grava_log("Interface Requisi$$HEX2$$e700e300$$ENDHEX$$o MRP - Erro alterar a DW [ds_ge473_lista_controles] - uo_ge473_requisicao_mrp_sap.of_processa_atualizacao" )
		Return
	End If
...
//				
//				Run("C:\Sistemas\EX\Exe\ex.exe /AUTO/SAP/" + String(il_tabela) + "/" + string(ll_nr_controle))
//				Continue
//				
//				ll_nr_controle = lds.Object.nr_controle[ll_Linha]
//				
...
//				Run("C:\Sistemas\EX\Exe\ex.exe /AUTO/SAP/" + String(il_tabela) + "/" + string(ll_nr_controle) + "/" + ls_nome_janela )
				...
				Try
					...
					lo_requisicao_mrp_sap.of_atualiza_requisicao_mrp_sap( lds.Object.nr_controle[ll_Linha])
	
				Finally
					Destroy(lo_requisicao_mrp_sap)
				End Try			
	...
	ElseIf ll_Linhas < 0 Then
		gvo_aplicacao.of_grava_log("Interface Requisi$$HEX2$$e700e300$$ENDHEX$$o MRP - Erro ao recuperar os da DW [ds_ge473_lista_controles] - uo_ge473_requisicao_mrp_sap.of_processa_atualizacao.")
	End If	
...
public function boolean of_atualiza_requisicao_mrp_sap (long al_controle);Boolean lb_Sucesso = False
...
Try
	...
	If lo_Comum.of_processa_carrega_dados(al_controle , ref ls_Log) Then
		...
		If ll_Linhas = 1 Then
			...
			if isvalid(w_aguarde_3) Then
				w_aguarde_3.wf_settext('Requisi$$HEX2$$e700e300$$ENDHEX$$o: ' + is_NR_Requisicao , 3 )
			end if
			...
			If This.of_valida_dados( Ref ls_Log) Then
				...
				If IsNumber(ls_Chave_Controle) Then
					If Long(ls_Chave_Controle) <> il_nr_requisicao Then
