...
public function boolean of_processa_carrega_dados (longlong all_controle, long al_tabela, ref string as_log)
...
try
	...
	Else
		as_log = "N$$HEX1$$e300$$ENDHEX$$o localizado a lista das colunas da tabela '" + String(al_tabela) + "' para montar a tabela tempor$$HEX1$$e100$$ENDHEX$$ria. Objeto [uo_ge473_comum], fun$$HEX2$$e700e300$$ENDHEX$$o [of_carrega_dados]." + 'Erro '  + SqlCa.SqlErrText
		Return False
	End If
	
	ls_Drop_Table = 'drop table #temptab1'
	Execute Immediate :ls_Drop_Table Using sqlca;
	If sqlca.sqlcode <> 0 Then
//		as_log = "Erro ao dropar a tabela tempor$$HEX1$$e100$$ENDHEX$$ria. Objeto [uo_ge473_comum], fun$$HEX2$$e700e300$$ENDHEX$$o [of_carrega_dados]." + 'Erro '  + SqlCa.SqlErrText
//		Return False
	End If
	
	Execute Immediate :ls_Create_Table Using sqlca;
	...
	If sqlca.sqlcode = 0 Then
		...
		Execute Immediate :ls_PK_TabTemp Using sqlca;
		
		If Sqlca.SqlCode <> 0 Then
			as_log = "Erro ao criar a chave prim$$HEX1$$e100$$ENDHEX$$ria tabela tempor$$HEX1$$e100$$ENDHEX$$ria. Objeto [uo_ge473_comum], fun$$HEX2$$e700e300$$ENDHEX$$o [of_carrega_dados]." + 'Erro '  + SqlCa.SqlErrText
			Return False
		End If		
	Else
		as_log = "Erro ao criar a tabela tempor$$HEX1$$e100$$ENDHEX$$ria. Objeto [uo_ge473_comum], fun$$HEX2$$e700e300$$ENDHEX$$o [of_carrega_dados]."
		Return False
	End If
	//FIM - CRIACAO TABELA TEMPORARIA
	
	// INICIO - ASSOCIAR OS CAMPOS DA TABELA TEMP NA DATA STORE QUE RECEBERA OS DADOS
	ls_Create_DS = SQLCA.SyntaxFromSQL("SELECT * FROM #temptab1 ",  "style(type=grid)", ls_Erro_Create_DS)
...
public function boolean of_processa_carrega_dados (longlong all_controle, long al_tabela, ref string as_log);If This.of_monta_ds_dinamica(all_controle, al_tabela, Ref as_log) Then
	If This.of_carrega_colunas_ds(Ref as_Log) Then
		If This.of_carrega_dados(all_controle, al_tabela, Ref as_log) Then
			Return True
		End If
	End If
End If
