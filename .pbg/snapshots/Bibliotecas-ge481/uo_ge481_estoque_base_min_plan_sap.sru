HA$PBExportHeader$uo_ge481_estoque_base_min_plan_sap.sru
forward
global type uo_ge481_estoque_base_min_plan_sap from uo_ge481_exportacao
end type
end forward

global type uo_ge481_estoque_base_min_plan_sap from uo_ge481_exportacao
end type
global uo_ge481_estoque_base_min_plan_sap uo_ge481_estoque_base_min_plan_sap
...
is_ds 						= 'ds_ge481_estoque_base_min_plan_sap'
...
Long		ll_qt_estoque_atual, ll_qt_estoque_minimo_matriz, ll_cd_promocao_planograma
String	ls_de_motivo, ls_cd_produto_sap, ls_cd_centro_sap, ls_nr_matricula_alteracao, &
			ls_id_alteracao, ls_cd_promocao_planograma, ls_cd_promocao_sap, ls_id_tipo_promocao
...
Try
	...
	ll_cd_promocao_planograma 		= pds_dados.Object.cd_promocao[pl_linha]
	ls_cd_promocao_sap 				= String(pds_dados.Object.cd_promocao_sap[pl_linha])
	ls_id_tipo_promocao				= pds_dados.Object.id_tipo_promocao[pl_linha]
	...
	if IsNull(ls_de_motivo) then ls_de_motivo = ''
	
	if IsNull(ll_cd_promocao_planograma) or ll_cd_promocao_planograma <= 0 then
		ls_cd_promocao_planograma	= ''
	else
		ls_cd_promocao_planograma	= String(ll_cd_promocao_planograma)
	end if
	
	ps_xml = 	'<Cabecalho>'
	ps_xml += 		'<id_tipo>'+ls_id_tipo_promocao+'</id_tipo>'
	ps_xml +=		'<Item>'
	ps_xml +=			'<cd_promocao_planograma>' + ls_cd_promocao_planograma + '</cd_promocao_planograma>'+&
							'<cd_promocao_sap>' + ls_cd_promocao_sap + '</cd_promocao_sap>'+&
...
on uo_ge481_estoque_base_min_plan_sap.create
call super::create
end on

on uo_ge481_estoque_base_min_plan_sap.destroy
call super::destroy
end on
