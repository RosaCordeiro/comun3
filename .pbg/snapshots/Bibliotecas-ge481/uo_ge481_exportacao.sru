HA$PBExportHeader$uo_ge481_exportacao.sru
forward
global type uo_ge481_exportacao from nonvisualobject
end type
end forward

global type uo_ge481_exportacao from nonvisualobject
end type
global uo_ge481_exportacao uo_ge481_exportacao
...
public function boolean _of_filtra_nr_atualizacao (ref dc_uo_ds_base pds_dados, string ps_nr_atualizacao, ref string ps_log)
public subroutine of_processa_envio (string ps_nr_atualizacao)
public function boolean of_atualiza_processado (datastore pds_itens, long pl_linha, string ps_nr_exportacao, string ps_status, ref string as_mensagem, ref string as_log)
public function boolean of_atualiza_processado (string ps_nr_exportacao, string ps_status, ref string as_mensagem, ref string ps_log)
end prototypes
...
protected function boolean of_valida_situacao (datastore pds_itens, long pl_linha, ref long pl_situacao_pendente, ref string ps_log);if not ib_validar_situacao then
	Select 1
	into :pl_situacao_pendente
	from log_exportacao
	where nr_exportacao = :is_nr_exportacao
	Using SQLCA;
else
	Select 1
	into :pl_situacao_pendente
	from log_exportacao
	where nr_exportacao = :is_nr_exportacao
	and id_situacao_exportacao = 'C'
	Using SQLCA;
end if

If SqlCa.SqlCode = -1 Then
	ps_log = "Erro ao consultar a tabela log_exportacao 'of_valida_situacao': " + SqlCa.SqlerrText
	Return False
End If
...
public function boolean _of_config_filtro_cab (datastore pds_cab, long pl_linha, ref datastore pds_itens, ref string ps_log);Return True
end function
...
public function boolean _of_processa_retorno_xml (string as_xml, ref string as_log);return true
end function

public function boolean of_atualiza_processado (datastore pds_itens, long pl_linha, string ps_status, ref string as_log);String	ls_mensagem

return this.of_atualiza_processado( pds_itens, pl_linha, '', ps_status, ref ls_mensagem, ref as_log)
end function

public function boolean of_atualiza_processado (datastore pds_itens, long pl_linha, ref string as_log);string ls_mensagem

return this.of_atualiza_processado( pds_itens, pl_linha, '', 'S', ref ls_mensagem, ref as_log)
end function
...
public subroutine of_processa_envio ();of_processa_envio('')
end subroutine

public function boolean _of_filtra_nr_atualizacao (ref dc_uo_ds_base pds_dados, string ps_nr_atualizacao, ref string ps_log);//Filtra a datastore para trazer um registro espec$$HEX1$$ed00$$ENDHEX$$fico.
if Not IsNull(ps_nr_atualizacao) and Trim(ps_nr_atualizacao) <> '' then
	pds_dados.of_appendwhere("nr_exportacao = '" + ps_nr_atualizacao + "'")
end if

return true
end function

public subroutine of_processa_envio (string ps_nr_atualizacao);String 	ls_grava_xml, ls_log, ls_xml, ls_xml_item, ls_xml_cab, ls_xml_retorno
Long 		ll_log, ll_linha, ll_linhas, ll_linhas_cab, ll_for, ll_seq_xml = 0, &
			ll_situacao_pendente, ll_progresso, ll_progresso_geral = 0, ll_qt_envios, &
			ll_nr_xml = 1, ll_contador
Dec{2}	ldc_divisao
...
try 
	...
	if not IsNull(ps_nr_atualizacao) and Trim(ps_nr_atualizacao) <> '' Then
		if this._of_filtra_nr_atualizacao( ref lds_itens, ps_nr_atualizacao, ref ls_log) = false then return 
	else
		if this._of_filtra_nr_atualizacao( ref lds_itens, '', ref ls_log) = false then return 
	end if
	...
	if ll_linhas < 0 then
		ls_log	= 'Erro ao carregar os dados da exporta$$HEX2$$e700e300$$ENDHEX$$o.'
	...
	if ll_linhas  > 0 Then		
		...
		For ll_linha = 1 to ll_linhas_cab
			...
			for ll_for = 1 to ll_linhas
				...
				is_nr_exportacao	= lds_itens.Object.nr_exportacao[ll_for]
				...
				if Not this._of_monta_xml_item( lds_itens, ll_for, ref ls_xml_item, ref ls_log) Then
					...
					of_atualiza_processado(lds_itens.Object.nr_exportacao[ll_for], 'E', ref ls_log, ref ls_log)
...
public function boolean of_atualiza_processado (datastore pds_itens, long pl_linha, string ps_nr_exportacao, string ps_status, ref string as_mensagem, ref string as_log);String 	ls_status, ls_mensagem_erro, ls_mensagem_sucesso, ls_nr_exportacao


if Trim(ps_nr_exportacao) = '' or IsNull(ps_nr_exportacao) Then
	ls_nr_exportacao = pds_itens.object.nr_exportacao[pl_linha]
else
	ls_nr_exportacao = ps_nr_exportacao
end if

if ps_status = 'E' Then
	ls_status = 'E'
	ls_mensagem_erro = as_mensagem
else
	ls_status = 'P'
	ls_mensagem_sucesso	= as_mensagem
end if

update log_exportacao
	set id_situacao_exportacao = :ls_status, 
		 dh_envio_webservice		= getdate(), 
		 de_erro 					= :ls_mensagem_erro,
		 de_retorno_webservice	= :ls_mensagem_sucesso,
		 cd_ambiente				= :is_Ambiente_SAP
 where nr_exportacao = :ls_nr_exportacao
Using SQLCA;

If SQLCA.SQLCode = -1 Then
	as_log = "Erro na atualiza$$HEX2$$e700e300$$ENDHEX$$o de processamento log_exportacao 'of_atualiza_processado': " + SQLCA.SQLErrText
	Return False
End If

as_mensagem	= 'Controle: ' + ls_nr_exportacao + ' - Problema: ' + ls_mensagem_erro

return true
end function

public function boolean of_atualiza_processado (string ps_nr_exportacao, string ps_status, ref string as_mensagem, ref string ps_log);datastore lds_nula

return this.of_atualiza_processado( lds_nula, 0, ps_nr_exportacao, ps_status, as_mensagem, ref ps_log ) 
end function

on uo_ge481_exportacao.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_ge481_exportacao.destroy
