HA$PBExportHeader$w_ge499_rel_resumo_venda_det_suma.srw
forward
global type w_ge499_rel_resumo_venda_det_suma from dc_w_selecao_lista_relatorio
end type
end forward
...
integer width = 3639
integer height = 1908
string title = "GE499 - Relat$$HEX1$$f300$$ENDHEX$$rio de Resumo Detalhado de Vendas Sumarizado"
end type
global w_ge499_rel_resumo_venda_det_suma w_ge499_rel_resumo_venda_det_suma
...
For lvl_Linha = 1 To dw_2.RowCount()
	...
	lvdt_Resumo = dw_2.Object.Dh_Resumo[lvl_Linha]

	If (IsNull(lvl_Filial) Or lvl_Filial = 0) And (IsNull(lvl_Regiao) Or lvl_Regiao = 0) Then
		...
		From nf_venda n
		Where n.cd_filial = (select distinct x.cd_filial
									from resumo_movimento_estoque x
									  where x.cd_filial = n.cd_filial
									  and x.dh_resumo = :lvdt_Resumo)
			 and n.dh_movimentacao_caixa = :lvdt_Resumo
			 and n.id_tipo_venda <> 'AV'
	...
	Else
		...
		Into 	:lvdc_CC_Dinheiro,
				...
				:lvdc_CV_Cartao_Credito
		From nf_venda n
		Where n.cd_filial in (select x.cd_filial
									from filial x
									  where x.cd_regiao = :lvl_Regiao)
			 and n.dh_movimentacao_caixa = :lvdt_Resumo
			 and n.id_tipo_venda <> 'AV'
			 and n.cd_forma_pagamento not in (n.id_tipo_venda, 'MF')
			 and n.dh_cancelamento is null
...
//	lvl_Filial = dw_2.Object.Cd_Filial[lvl_Linha]
	...
	If (IsNull(lvl_Filial) Or lvl_Filial = 0) And (IsNull(lvl_Regiao) Or lvl_Regiao = 0) Then
		...
		where cx.cd_filial 					= f.cd_filial
		  ...
		  and f.cd_filial 						= (select distinct x.cd_filial
													from resumo_movimento_estoque x
													  where x.cd_filial = f.cd_filial
													  and x.dh_resumo = :lvdt_Resumo)
		  and cc.dh_movimentacao_caixa = :lvdt_Resumo
...
on w_ge499_rel_resumo_venda_det_suma.create
call super::create
end on

on w_ge499_rel_resumo_venda_det_suma.destroy
call super::destroy
...
event ue_preopen;call super::ue_preopen;MaxWidth	= 5285
MaxHeight	= 9999
...
type dw_visual from dc_w_selecao_lista_relatorio`dw_visual within w_ge499_rel_resumo_venda_det_suma
end type

type gb_aux_visual from dc_w_selecao_lista_relatorio`gb_aux_visual within w_ge499_rel_resumo_venda_det_suma
end type

type gb_2 from dc_w_selecao_lista_relatorio`gb_2 within w_ge499_rel_resumo_venda_det_suma
integer x = 59
integer y = 212
integer width = 3502
integer height = 1480
...
type gb_1 from dc_w_selecao_lista_relatorio`gb_1 within w_ge499_rel_resumo_venda_det_suma
integer x = 41
integer width = 3515
integer height = 184
end type

type dw_1 from dc_w_selecao_lista_relatorio`dw_1 within w_ge499_rel_resumo_venda_det_suma
integer x = 82
integer width = 3447
integer height = 84
string dataobject = "dw_ge499_selecao"
end type
...
type dw_2 from dc_w_selecao_lista_relatorio`dw_2 within w_ge499_rel_resumo_venda_det_suma
integer x = 91
integer y = 288
integer width = 3438
integer height = 1372
string dataobject = "dw_ge499_lista"
...
lvs_Coluna = {"dh_resumo"}

lvs_Nome = {"Per$$HEX1$$ed00$$ENDHEX$$odo"}
...
type dw_3 from dc_w_selecao_lista_relatorio`dw_3 within w_ge499_rel_resumo_venda_det_suma
integer x = 1733
integer y = 876
string dataobject = "dw_ge499_relatorio"
end type
