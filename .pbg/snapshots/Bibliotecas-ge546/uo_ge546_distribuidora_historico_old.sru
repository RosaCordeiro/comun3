HA$PBExportHeader$uo_ge546_distribuidora_historico_old.sru
forward
global type uo_ge546_distribuidora_historico_old from nonvisualobject
end type
end forward

global type uo_ge546_distribuidora_historico_old from nonvisualobject
end type
global uo_ge546_distribuidora_historico_old uo_ge546_distribuidora_historico_old
...
// todos os dados da distribuidora_produto serao inseridos na distribuidora_produto_historico uma vez por dia (este processo rodara uma vez por dia)
...
Date 		lvdt_movimento, lvdt_data_ultima_carga
...
select cd_distribuidora,
		 ...
		 vl_icms_st_farmacia_popular,
		 de_codigo_barras_arquivo_preco,
		 isnull(qt_embalagem_padrao_distrib,1),
		 qt_estoque_disponivel,
		 dh_atualizacao_estoque,
		 id_estoque_maior_10dias,
		 dh_arquivo_estoque,
		 ...
		 nr_embalagem_distrib,
		 dh_exportacao_sap,
		 coalesce(round((round( (case when cd_distribuidora = '053404705'   
	            then ((IsNull(vl_preco_novo, vl_preco_atual) * 0.05) + IsNull(vl_preco_novo, vl_preco_atual))    
	            else IsNull(vl_preco_novo, vl_preco_atual) end) * 
	    ((100 - IsNull(pc_desconto_novo, pc_desconto_atual) ) / 100), 2) - 
	            round( round( case when cd_distribuidora = '053404705'   
	           				  then ((IsNull(vl_preco_novo, vl_preco_atual) * 0.05) + IsNull(vl_preco_novo, vl_preco_atual))    
	                		  else IsNull(vl_preco_novo, vl_preco_atual) end * 
	    ((100 - IsNull(pc_desconto_novo, pc_desconto_atual) ) / 100), 2) * (pc_repasse_icms / 100), 2) + vl_icms_st) / isnull(qt_embalagem_padrao_distrib,1) ,2), 0)
...
// Na criacao deste processo de carga da tabela destino (distribuidora_produto_hist), esta possui 50 campos.
// Ja a tabela de origem (distribuidora_produto), possui atualmente 51 campos.
...
if lvl_count_colunas_destino - 1 <> lvl_count_colunas_origem then
	ab_divergencia = true
	as_mensagem += 'N$$HEX1$$e300$$ENDHEX$$o foi poss$$HEX1$$ed00$$ENDHEX$$vel iniciar o processamento do hist$$HEX1$$f300$$ENDHEX$$rico dos produtos importados das distribuidoras. H$$HEX1$$e100$$ENDHEX$$ diverg$$HEX1$$ea00$$ENDHEX$$ncias entre as colunas da tabela origem (distribuidora_produto) e destino (distribuidora_produto_hist). ' + &
						'Verifique se novas colunas foram adicionadas recentemente na tabela origem.'
end if
...
on uo_ge546_distribuidora_historico_old.create
call super::create
TriggerEvent( this, "constructor" )
end on

on uo_ge546_distribuidora_historico_old.destroy
